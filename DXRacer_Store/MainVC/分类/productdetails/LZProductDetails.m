...
- (void)updateUIWithImageAndVideoArray:(NSArray *)detailsArr{
    ...
    self.scrolV.contentSize = CGSizeMake(detailsArr.count*SCREEN_WIDTH, SCREEN_WIDTH);
    ...
    for (int i = 0; i < detailsArr.count; i ++) {
        //NSLog(@"+++++++++++++++++%@",detailsArr[i]);
        if (i == 0) {
//            self.videoCoverImgV.frame = CGRectMake(i*self.frame.size.width, 0, SCREEN_WIDTH, SCREEN_WIDTH);
...
//            [self.scrolV addSubview:self.webview];
            ...
            [self.scrolV addSubview:self.imgV];
        }
...
//    NSLog(@"+++++++-------%ld",sender.view.tag);
    ...
    if (self.scrollOptBlock) {
        self.scrollOptBlock(sender.view.tag);
    }
}
