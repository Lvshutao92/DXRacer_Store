...
- (UIProgressView *)myProgressView {
    if (!_myProgressView) {
        ...
        //设置进度条颜色
        _myProgressView.trackTintColor = [UIColor lightGrayColor];
        //设置进度条上进度的颜色
        _myProgressView.progressTintColor = RGBACOLOR(49, 184, 243, 1);
    }
...
//                    [[NSNotificationCenter defaultCenter] postNotification:notification];
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (days==0) {
                            ...
                            strongSelf.dayLabel.text = [NSString stringWithFormat:@"%d天",days];
                        }
                        if (hours<10) {
                            strongSelf.hourLabel.text = [NSString stringWithFormat:@"0%d",hours];
                        }else{
                            strongSelf.hourLabel.text = [NSString stringWithFormat:@"%d",hours];
                        }
                        if (minute<10) {
                            strongSelf.minuteLabel.text = [NSString stringWithFormat:@"0%d",minute];
                        }else{
                            strongSelf.minuteLabel.text = [NSString stringWithFormat:@"%d",minute];
                        }
                        if (second<10) {
                            strongSelf.secondLabel.text = [NSString stringWithFormat:@"0%d",second];
                        }else{
                            strongSelf.secondLabel.text = [NSString stringWithFormat:@"%d",second];
                        }
